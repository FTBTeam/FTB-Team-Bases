...
### Added
* Added new base placement mode - spiral - in addition to existing "march east" placement mode
  * Spiral is default, see `allocation_mode` setting in server configuration

## [21.1.16]

### Changed
* Player velocity is reset to 0 immediately after a teleport (`/ftbteambases home` and `lobby` commands)
  * Avoids nasty surprises if teleporting from a moving aeronautics contraption

## [21.1.15]

### Fixed
* Various issues with pregen copying and lobby copying
...
### Fixed
* Fixed lobby autoclaiming not working correctly
...
### Fixed
* Resolved a weird edge-case with dimension loading + teleporting. This could cause players to get stuck in a weird timing state.
