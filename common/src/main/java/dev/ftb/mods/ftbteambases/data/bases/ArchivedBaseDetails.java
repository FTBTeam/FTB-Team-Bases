...
public record ArchivedBaseDetails(String archiveId, RegionExtents extents, ResourceKey<Level> dimension, BlockPos spawnPos, UUID ownerId, long archiveTime) {
    public static final Codec<ArchivedBaseDetails> CODEC = RecordCodecBuilder.create(inst -> inst.group(
            Codec.STRING.fieldOf("id").forGetter(ArchivedBaseDetails::archiveId),
    ...
    public LiveBaseDetails makeLiveBaseDetails() {
        return new LiveBaseDetails(extents, dimension, spawnPos);
    }
}
