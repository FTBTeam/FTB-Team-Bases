...
public interface ConstructionWorker {
     ...
     * {@code exact_spawn} position, that position is returned directly with no further calculation. Otherwise,
    ...
    default BlockPos getInitialSpawnPos(Level destLevel, BaseDefinition baseDefinition) {
        if (baseDefinition.exactSpawn().isPresent()) {
            return baseDefinition.exactSpawn().get();
        }
