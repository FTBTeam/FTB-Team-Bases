...
public record SingleStructure(Identifier structureLocation, Optional<Integer> yPos, boolean includeEntities) implements INetworkWritable<SingleStructure> {
    public static final Codec<SingleStructure> CODEC = RecordCodecBuilder.create(inst -> inst.group(
            Identifier.CODEC.fieldOf("structure_location").forGetter(SingleStructure::structureLocation),
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, SingleStructure> STREAM_CODEC = StreamCodec.composite(
            Identifier.STREAM_CODEC, SingleStructure::structureLocation,
